    ...
    def _run_buy(self, procurements):
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            if not po:
                ...
                # Get the min date and update if necessary
                date_order = min(vals.get('date_order'), po.date_order)
                if date_order and po.date_order != date_order:
                    po.write({"date_order": date_order})
                # If a purchase order is found, adapt its `origin` field.
                if po.origin:
                    missing_origins = origins - set(po.origin.split(', '))
                    if missing_origins:
                        po.write({
                                     'origin': po.origin + ', ' + ', '.join(
                                         missing_origins)
                                 })
