...
class AutomaticEntryWizard(models.TransientModel):
    ...
    def _get_move_line_dict_vals_change_period(self, aml):
        ...
        if self.account_type == 'income':
            accrual_account = self.revenue_accrual_account
        else:
            accrual_account = self.expense_accrual_account
    ...
    def _get_move_dict_vals_change_period(self):
        ...
        for aml in self.move_line_ids:
            ...
            move_line_data = self._get_move_line_dict_vals_change_period(aml)
            move_data['new_date']['line_ids'] += move_line_data[0]
            move_data[aml.move_id.date]['line_ids'] += move_line_data[1]
